  ...
  return (
    <div className="arrodes-stage" data-mode={mode} data-active={active ? '1' : '0'}>
      <div
        ...
        role="button"
        tabIndex={0}
        title="Hover for the glass. Click for a puddle."
...
void main() {
  ...
  float flow = t * (0.14 + listen * 0.10 + speak * 0.24 + hover * 0.08);
  ...
  float gleamBand = exp(-dHover * 4.6) * hover;
  ...
  pane = mix(pane, gleam, pow(smoothstep(0.74, 1.08, h + speak * 0.08 + hover * 0.16), 1.7));

  float spec = pow(max(0.0, 1.0 - abs(h - 0.76 - uAmp * 0.1)), 10.0);
  spec += pow(max(0.0, sin(uv.x * 7.0 + n * 5.0 - t * 0.9) * 0.5 + 0.5), 14.0) * 0.32;
  spec += hover * pow(max(0.0, 1.0 - abs(uv.x - ho.x)), 6.0) * 0.7;
  pane += gleam * spec * (0.28 + speak * 0.22 + listen * 0.10 + hover * 0.42);
  pane += gleam * gleamBand * 0.55;
  pane += gleam * abs(wave) * 0.85;
  ...
  orb = mix(orb, silver, 0.20 + 0.32 * sheet + 0.18 * n2 + hover * 0.16);
  orb = mix(orb, gleam, 0.16 * speak * uAmp + 0.10 * rings + hover * 0.12);

  vec3 col = pane;
  col = mix(col, orb, core * 0.92);
  col += gleam * rim * (0.26 + 0.38 * speak + hover * 0.22);
  col += mercury * glow * (0.18 + 0.16 * (1.0 - speak));
  col += gleam * pow(glow, 2.15) * (0.08 + 0.22 * speak * uAmp + splash * 0.2);
