...
export default function ArrodesVoiceMirror({
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      resize();
      draw(gl, prog, canvas, (now - startRef.current) / 1000, modeRef.current, bandsRef.current, hoverRef.current, hoverPtRef.current, splashRef.current);
    };
    rafRef.current = requestAnimationFrame(tick);
    resize();
  ...
  return (
    <div className="arrodes-stage" data-mode={mode} data-active={active ? '1' : '0'} data-expanded={expanded ? '1' : '0'} data-variant={variant}>
      <div
        ...
        onPointerMove={(e) => { hoverRef.current = 1; hoverPtRef.current = pointInWell(e); }}
