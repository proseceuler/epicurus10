...
export default function GlobalAssistant({
  ...
  const send = async (text?: string) => {
    ...
    if (!key) {
      setError('Add an OpenRouter key in Settings first.');
      return;
    }
    setError('');
    setInput('');
  ...
  return (
    <>
      {open && (
        <div className="fixed inset-0 z-40 bg-zinc-900/20 backdrop-blur-[2px] lg:hidden" onClick={onClose} />
      )}
      <aside
        className={`assistant-panel fixed z-50 flex flex-col bg-white/80 shadow-[-12px_0_40px_rgba(0,0,0,0.08)] transition-transform duration-300 ease-out ${
          open ? 'translate-x-0' : 'translate-x-full'
        } inset-y-0 right-0 w-full max-w-none lg:top-3 lg:bottom-3 lg:right-3 lg:w-[340px] lg:max-w-[340px] lg:rounded-[22px] lg:border lg:border-white/60`}
      >
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-zinc-400">{pageLabel(page)}</p>
            </div>
          </div>
          <button type="button" onClick={onClose} className="rounded-full p-1.5 text-zinc-500 hover:bg-zinc-100">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div ref={scrollRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-2">
          {messages.length === 0 && (
            <p className="pt-6 text-sm leading-relaxed text-zinc-500">
              Ask about grades, tasks, schedule, or notes. I can suggest a next step — nothing is added until you confirm.
            </p>
          )}
          {messages.map((m, i) => (
            <div key={i} className={m.role === 'user' ? 'ml-6 rounded-2xl bg-zinc-900 px-3 py-2 text-sm text-white' : 'mr-2 text-sm text-zinc-700'}>
              ...
              {m.pending && (
                <div className="mt-3 flex gap-2">
                  <button type="button" onClick={() => confirmWrite(i, true)} className="inline-flex items-center gap-1 rounded-full bg-zinc-900 px-3 py-1 text-xs text-white">
                    <Check className="h-3 w-3" /> Confirm
                  </button>
                  <button type="button" onClick={() => confirmWrite(i, false)} className="inline-flex items-center gap-1 rounded-full bg-zinc-100 px-3 py-1 text-xs text-zinc-600">
        ...
        <form
          className="border-t border-zinc-200/70 p-3"
          onSubmit={(e) => {
            e.preventDefault();
            void send();
          }}
        >
          <div className="flex items-end gap-2 rounded-2xl bg-zinc-100/80 px-3 py-2">
            ...
            />
            <button type="submit" disabled={busy || !input.trim()} className="flex h-8 w-8 items-center justify-center rounded-full bg-zinc-900 text-white disabled:opacity-30">
...
function pageLabel(page: PageId) {
  return page.replace(/^\w/, (c) => c.toUpperCase());
}
...
async function ask(history: Msg[], page: PageId, key: string): Promise<Msg> {
  ...
  const system = [
    ...
    'Stay on study and school productivity. Do not write or edit code.',
    pageContext(page),
    'You may read grades, tasks, calendar, notes, habits, focus and timetable with tools.',
    'If the student wants something created, call the matching write tool. The app will ask them to confirm before anything is saved. Do not claim it was saved already.',
    'Be concise and calm.',
  ].join(' ');
  ...
  const body = {
    ...
    tools: [...READ_TOOLS, ...CONFIRM_TOOLS],
    temperature: 0.4,
  };
  ...
  if (toolCalls?.length) {
    ...
    if (writes[0]) {
      ...
      return {
        role: 'assistant',
        content: `${choice?.content || 'I can do that if you confirm.'}\n\nProposed: **${writes[0].function.name}**`,
        pending: { name: writes[0].function.name, args },
      };
    }
    if (snippets.length) {
