...
void main() {
  ...
  float gleamBand = exp(-dHover * 5.4) * hover * 0.30;
  float gleamSweep = exp(-abs(uv.x - mix(0.18, ho.x, hover)) * 7.2) * hover * 0.24;
  float gleamRidges = pow(max(0.0, 1.0 - abs(uv.y - mix(0.22, ho.y, hover))), 10.0) * hover * 0.10;
  ...
  pane = mix(pane, gleam, pow(smoothstep(0.74, 1.08, h + speak * 0.08 + hover * 0.08), 1.7));
  float spec = pow(max(0.0, 1.0 - abs(h - 0.76 - uAmp * 0.1)), 10.0);
  spec += pow(max(0.0, sin(uv.x * 7.0 + n * 5.0 - t * 0.9) * 0.5 + 0.5), 14.0) * 0.22;
  spec += hover * pow(max(0.0, 1.0 - abs(uv.x - ho.x)), 7.0) * 0.22;
  pane += gleam * spec * (0.16 + speak * 0.12 + listen * 0.06 + hover * 0.16);
  pane += gleam * (gleamBand + gleamSweep + gleamRidges);
  ...
  orb = mix(orb, silver, 0.20 + 0.32 * sheet + 0.18 * n2 + hover * 0.07);
  orb = mix(orb, gleam, 0.16 * speak * uAmp + 0.10 * rings + hover * 0.06);
  vec3 col = pane;
  col = mix(col, orb, core * 0.92);
  col += gleam * rim * (0.18 + 0.28 * speak + hover * 0.10);
  col += mercury * glow * (0.18 + 0.16 * (1.0 - speak));
  col += gleam * pow(glow, 2.15) * (0.06 + 0.16 * speak * uAmp + splash * 0.14 + hover * 0.08);
