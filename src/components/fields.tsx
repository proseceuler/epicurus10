...
function Menu({
  ...
  const [pos, setPos] = useState({ top: 0, left: 0, width: 220, maxH: 320 });
  ...
  useLayoutEffect(() => {
    ...
    const place = () => {
      ...
      const desired = Math.max(80, menuRef.current?.offsetHeight || estHeight);
      let openDown: boolean;
      if (prefer === 'down') openDown = spaceBelow >= 120 || spaceBelow >= spaceAbove;
      else if (prefer === 'up') openDown = spaceAbove < 120 && spaceBelow > spaceAbove;
      else openDown = spaceBelow >= desired || spaceBelow >= spaceAbove;
      const avail = openDown ? spaceBelow : spaceAbove;
      const maxH = Math.max(96, Math.min(desired, avail || desired));
      const top = openDown ? r.bottom + gap : Math.max(pad, r.top - maxH - gap);
      let left = r.left;
...
export function TimeField({
  ...
  return (
    <div ref={triggerRef} className={`relative ${className}`}>
      ...
      <Menu open={open} triggerRef={triggerRef} menuRef={menuRef} width={228} estHeight={220} prefer="down" className="p-2">
