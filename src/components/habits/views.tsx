      ...
      <div className="space-y-3">
                ...
                return (
                  <li
                    ...
                    className={`flex items-center gap-2 rounded-xl px-2.5 py-2 text-sm ${
                      ok ? 'bg-zinc-100 text-zinc-500' : 'text-zinc-800'
                    }`}
                  >
        ...
        <div className="glass rounded-2xl p-4">
          ...
          <div className="flex items-end gap-1.5">
            {dailyScores.slice(-7).map((d) => (
              <div key={d.date} className="flex flex-1 flex-col items-center gap-1">
                <div
                  className="w-full rounded-md bg-zinc-900/80"
                  style={{ height: `${Math.max(4, d.score * 48)}px` }}
          ...
          <p className="mt-2 text-[10px] text-zinc-400">
            {completions.length} total check-ins logged
          </p>
        </div>
                    ...
                    {recent.map((d) => (
                      <span
                        ...
                        className={`h-2 w-2 rounded-sm ${
                          isDone(done, h.id, d) ? 'bg-zinc-800' : 'bg-zinc-200'
                        }`}
                      />
                    ))}
                  </div>
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs text-zinc-500">
          Click a cell to toggle · {days.length} days in view
        </p>
        <button
          type="button"
        ...
        <div className="glass flex flex-wrap items-end gap-2 rounded-2xl p-3">
          <label className="text-xs text-zinc-500">
            ...
            <input
              ...
              className="mt-1 block w-14 rounded-lg border border-zinc-200 bg-white px-2 py-1.5 text-sm"
            />
          </label>
          ...
          <label className="text-xs text-zinc-500">
            ...
            <input
              ...
              className="mt-1 block w-16 rounded-lg border border-zinc-200 bg-white px-2 py-1.5 text-sm"
            />
          </label>
          <button
            type="button"
          ...
          <button
            ...
            className="rounded-xl px-3 py-1.5 text-xs text-zinc-500 hover:bg-zinc-100"
          >
            Cancel
          </button>
        </div>
      ...
      <div className="glass overflow-x-auto rounded-2xl p-3">
        <table className="w-full min-w-[640px] border-collapse text-xs">
          <thead>
            <tr>
              <th className="sticky left-0 z-10 bg-white/90 px-2 py-1.5 text-left font-medium text-zinc-600">
                Habit
              </th>
              {days.map((d) => (
                <th
                  ...
                  className={`px-0.5 py-1 text-center font-normal ${
                    d.dateStr === today ? 'text-zinc-900' : 'text-zinc-400'
                  }`}
                >
          ...
          <tbody>
                  ...
                  <td className="sticky left-0 z-10 bg-white/90 px-2 py-1.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="tabular-nums text-[10px] text-zinc-400">
                        {monthHits}/{h.goal_target || days.length}
                      </span>
                    </div>
                  </td>
                  {days.map((d) => {
                    ...
                    return (
                      <td key={d.dateStr} className="px-0.5 py-1 text-center">
                        <button
                          type="button"
                          onClick={() => onToggle(h.id, d.dateStr)}
                          className={`mx-auto h-5 w-5 rounded-md border transition-colors ${
                            ok
                              ? 'border-zinc-800 bg-zinc-800'
                              : 'border-zinc-200 bg-white hover:border-zinc-400'
                          } ${d.dateStr === today ? 'ring-1 ring-zinc-400 ring-offset-1' : ''}`}
                          aria-label={`${h.name} ${d.dateStr}`}
                        />
                      </td>
                    );
                  })}
            ...
            {habits.length === 0 && (
              <tr>
                <td colSpan={days.length + 2} className="px-2 py-8 text-center text-zinc-400">
        ...
        {/* weeks used for layout parity / future weekly grouping */}
    ...
    <div className="space-y-3">
                ...
                {days.map((d) => (
                  <span
                    ...
                    className={`h-2.5 w-2.5 rounded-sm ${
                      isDone(done, h.id, d.dateStr) ? 'bg-zinc-800' : 'bg-zinc-200'
                    }`}
                    title={d.dateStr}
                  />
                ))}
      ...
      <div className="glass rounded-2xl p-4">
        ...
        <div className="space-y-2">
          {weeks.map((week, wi) => {
            const score =
              habits.length && week.length
                ? week.reduce(
                    (acc, d) =>
                      acc +
                      habits.filter((h) => isDone(done, h.id, d.dateStr)).length / habits.length,
                    0,
                  ) / week.length
                : 0;
            return (
...
export function InsightsView({
  ...
  const series = habits.map((h) =>
    days.map((d) => (isDone(done, h.id, d.dateStr) ? 1 : 0)),
  );
  ...
  const byWeekday = WEEKDAYS.map((_, wi) => {
    ...
    const hits = subset.reduce(
      (acc, d) => acc + habits.filter((h) => isDone(done, h.id, d.dateStr)).length,
      0,
    );
  ...
  return (
    <div className="grid gap-3 lg:grid-cols-2">
      <div className="glass rounded-2xl p-4">
        ...
        <div className="flex items-end gap-2" style={{ height: 120 }}>
          {byWeekday.map((v, i) => (
            <div key={WEEKDAYS[i]} className="flex flex-1 flex-col items-center justify-end gap-1">
              <div
                className="w-full max-w-[2rem] rounded-t-md bg-zinc-800"
                style={{ height: `${Math.max(4, v * 100)}px` }}
              />
        ...
        <p className="mt-2 text-[10px] text-zinc-400">
          Share of habits completed on each weekday this month.
        </p>
      </div>
              ...
              <li key={`${p.a.id}-${p.b.id}`} className="flex items-center gap-2 text-xs">
                ...
                <span
                  className={`tabular-nums font-medium ${
                    p.r > 0.3 ? 'text-zinc-800' : p.r < -0.3 ? 'text-zinc-500' : 'text-zinc-400'
                  }`}
                >
                  {p.r >= 0 ? '+' : ''}
                  {p.r.toFixed(2)}
        ...
        <div className="flex flex-wrap gap-1">
          {days.map((d) => {
            ...
            return (
              <div
                key={d.dateStr}
                title={`${d.dateStr}: ${Math.round(t * 100)}%`}
                className="flex h-8 w-8 items-center justify-center rounded-md text-[10px] tabular-nums text-zinc-600"
                style={{ background: greyFill(t) }}
              >
                {d.day}
              </div>
            );
