        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={r.key}>
              ...
              {grid[i].map((v, j) => (
                <td key={cols[j].key} className="px-1 py-0.5 text-center" style={{ background: heatGrey(v / colMax), color: v / colMax > 0.55 ? '#fafafa' : '#27272a' }}>{Number.isInteger(v) ? v : v.toFixed(0)}</td>
              ))}
    ...
    <div className="overflow-x-auto">
      <div className="min-w-max">
              ...
              {cols.map((c) => {
                ...
                return (
                  <div
                    ...
                    style={{ width: cell, height: cell, background: on ? heatGrey(0.82) : '#f4f4f5', color: on ? '#fafafa' : '#a1a1aa' }}
                  >{numbers ? (on ? '1' : '0') : on ? '' : ''}</div>
                );
              })}
        ...
        <div className="mt-px flex items-center gap-px">
          ...
          {totals.map((t, i) => (
            <div key={cols[i].dateStr} className="flex items-center justify-center rounded-[2px] text-[7px] tabular-nums" style={{ width: cell, height: cell, background: heatGrey(t), color: t > 0.55 ? '#fafafa' : '#3f3f46' }}>
              {Math.round(t * 100)}
            </div>
          ))}
        ...
        {rows.map((r) => (
          <div key={r.key} className="flex items-center gap-px">
            ...
            {cols.map((c) => {
              ...
              return (
                <div key={c.key} className="flex items-center justify-center rounded-[2px] text-[8px] tabular-nums" style={{ width: cell, height: cell, background: heatGrey(v), color: v > 0.55 ? '#fafafa' : '#3f3f46' }}>
                  {Math.round(v * 100)}
                </div>
              );
            })}
      ...
      {Object.values(byWeek).map((week, i) => (
        <div key={i} className="flex items-center gap-px">
          ...
          {Array.from({ length: 7 }, (_, wd) => {
            ...
            return <div key={cell.dateStr} className="flex h-5 w-5 items-center justify-center rounded-[2px] text-[7px] tabular-nums" style={{ background: heatGrey(pct), color: pct > 0.55 ? '#fafafa' : '#3f3f46' }}>{Math.round(pct * 100)}</div>;
          })}
        </div>
      ))}
    </div>
