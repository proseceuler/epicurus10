...
export function MiniSheet({ title, open, onClose, children, align = 'left' }: {
  ...
  return (
    <MotionPopover open={open} className={`absolute bottom-[calc(100%+8px)] z-40 w-[min(92vw,20.5rem)] overflow-visible p-0 ${align === 'right' ? 'right-0' : 'left-0'}`}>
      <div ref={ref} className="relative max-h-[min(22rem,calc(100vh-8rem))] overflow-y-auto rounded-[1.15rem]">
