...
export type NodeKind = 'index' | 'daily' | 'topic' | 'board' | 'unresolved' | 'tag';
...
export const GRAPH_SETTINGS_KEY = 'epicure:graph-settings:v3';
export const GRAPH_PINS_KEY = 'epicure:graph-pins:v1';
...
export function loadGraphSettings(): GraphSettings {
  try {
    const raw = localStorage.getItem(GRAPH_SETTINGS_KEY) || localStorage.getItem('epicure:graph-settings:v2') || localStorage.getItem('epicure:graph-settings:v1');
...
export function loadPins(): Record<string, { x: number; y: number }> {
  try {
    const raw = localStorage.getItem(GRAPH_PINS_KEY);
...
export function savePins(pins: Record<string, { x: number; y: number }>) {
  try {
    localStorage.setItem(GRAPH_PINS_KEY, JSON.stringify(pins));
  } catch {
    /* */
  }
}
...
export function buildNotesGraph(opts: {
    ...
    if (gset.showTags) {
      for (const raw of n.tags || []) {
        ...
        if (!all.some((x) => x.id === id)) {
          all.push({ id, label: `#${tag}`, kind: 'tag', degree: 0, inDegree: 0, outDegree: 0 });
        }
        rawLinks.push({ from: n.id, to: id });
      }
    }
  ...
  let ns = all.filter((n) => n.kind === 'unresolved' || n.kind === 'tag' || enabled.has(n.kind));
  if (!gset.showTags) ns = ns.filter((n) => n.kind !== 'tag');
  ...
  if (gset.mode === 'local') {
    ...
    if (seed) {
      ...
      let kept = ls.filter((l) => vis.has(String(l.source)) && vis.has(String(l.target)));
      if (!gset.neighborLinks) {
        kept = kept.filter((l) => String(l.source) === seed || String(l.target) === seed);
      }
      return { nodes: ns, links: kept };
    }
  }
