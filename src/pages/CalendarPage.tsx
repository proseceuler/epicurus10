...
export default function CalendarPage() {
  ...
  return (
    <div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/30 p-4 backdrop-blur-sm" onClick={() => setShowForm(false)}>
          <div className="glass glass-shadow-lg max-h-[88vh] w-full max-w-md overflow-y-auto rounded-2xl p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="mb-1 block text-xs text-zinc-500">Starts</label><input type="date" value={draft.start_date} onChange={(e) => setDraft({ ...draft, start_date: e.target.value, end_date: draft.end_date < e.target.value ? e.target.value : draft.end_date })} className="glass-input w-full rounded-xl px-3 py-2 text-sm text-zinc-800" /></div>
                <div><label className="mb-1 block text-xs text-zinc-500">Ends</label><input type="date" value={draft.end_date} onChange={(e) => setDraft({ ...draft, end_date: e.target.value })} className="glass-input w-full rounded-xl px-3 py-2 text-sm text-zinc-800" /></div>
              </div>
              ...
              {!draft.all_day && (
                <div className="grid grid-cols-2 gap-3">
                  <input type="time" value={draft.start_time} onChange={(e) => setDraft({ ...draft, start_time: e.target.value })} className="glass-input w-full rounded-xl px-3 py-2 text-sm text-zinc-800" />
                  <input type="time" value={draft.end_time} onChange={(e) => setDraft({ ...draft, end_time: e.target.value })} className="glass-input w-full rounded-xl px-3 py-2 text-sm text-zinc-800" />
                </div>
              )}
              <div className="grid grid-cols-2 gap-3">
