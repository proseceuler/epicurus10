          ...
          <div className="space-y-1">
            {NOTE_TEMPLATES.map((tmpl) => (
              <button key={tmpl.id} type="button" className="flex w-full flex-col rounded-lg px-2.5 py-1.5 text-left hover:bg-zinc-100" onClick={() => {
                const title = tmpl.id === 'reflection' ? new Date().toISOString().slice(0, 10) : tmpl.title;
                void onCreate({ title: title || tmpl.name, folder: tmpl.folder || 'Vault', content: tmpl.content });
              }}>
        ...
        {showImport && onImportNotes ? (
          ...
          <div className="glass rounded-2xl p-5">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Button onClick={async () => { if (!newTitle.trim()) return; await onCreate({ title: newTitle.trim(), folder: newFolder || 'Vault', content: '' }); setShowNew(false); }}>Create</Button>
          ...
          <div className="glass flex h-full min-h-[70vh] flex-col rounded-2xl">
              ...
              <div className="relative">
                ...
                {linkPicker && (
                  <div className="absolute right-0 top-full z-20 mt-1 max-h-56 w-56 overflow-y-auto rounded-xl border border-zinc-200 bg-white shadow-lg">
                    ...
                    <button type="button" onClick={() => insertWiki('Board: Study sketch')} className="flex w-full items-center gap-2 px-3 py-2 text-left text-xs hover:bg-zinc-100">
            ...
            <div className="min-h-0 flex-1 overflow-hidden">
              {editMode ? (
                <textarea ref={taRef} value={draft.content} onChange={(e) => persistDraft({ ...draft, content: e.target.value })} placeholder="Write in markdown. Use [[Note title]] or [[Board: Name]] to link." className="h-full min-h-[28rem] w-full resize-none bg-transparent px-5 py-4 font-mono text-sm leading-relaxed outline-none" />
              ) : (
